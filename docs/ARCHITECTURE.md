...
- **鼠标手势**：滚轮切合约、左/右键切分组、左键**拖动**仍是移动面板、中键开菜单。
  单击与拖动按**移动距离**区分（>4px 才算拖）——拖动是面板唯一的定位方式，不能让给单击；
  而按下就 `DragMove()` 会把单击整个吞掉（这也是原来"单击毫无反馈"的原因）。
  右键让位给切分组，所以菜单挪到中键；还原主窗口仍有托盘图标双击这条路。
