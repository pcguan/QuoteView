...
- **重建后必须 `Mouse.Synchronize()`**：`Render` 每次都丢弃并重建所有行，光标下的 TextBlock 随之
  离开可视树；滚轮是**路由事件**，起点就是 mouse-over 元素，元素没了事件就到不了窗口。光标不动系统也
  不会重测，于是"停在原地滚几下就失灵、动一下鼠标又好"。重建后强制重做命中测试即可。
  （这才是真凶；下面两条是先修掉的另外两条独立失效路径。）
